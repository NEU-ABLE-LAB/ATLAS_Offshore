...
switch mlc.parameters.evaluation_method
    ...
    case 'standalone_function'
        ...
        for i=istart:length(eval_idx);
            ...
            JJ(i)=feval(f,m,mlc.parameters,eval_idx(i));
        end
        case 'standalone_function_generation'
        %% only one evaluator is used, the function given in
        %% mlc.parameters.evaluation_fuction is used.
        eval(['heval=@' mlc.parameters.evaluation_function ';']);
        f=heval;
        Js=feval(f,idv_to_evaluate,mlc.parameters,istart);
        JJ(istart:length(eval_idx))=Js(istart:length(eval_idx));
...
if  mlc.parameters.saveincomplete==1 && ~strcmp(mlc.parameters.evaluation_method,'multithread_function')...
        && ~strcmp(mlc.parameters.evaluation_method,'standalone_function_generation');
